...
import { readUserContext, writeUserContext } from "../system/ai/contextProvider.js";
import path from "path";
import fs from "fs";
import axios from "axios";
import { formatSize, rand, getGreeting, date, time } from "../system/function.js";
import { setupAutomatedBriefing, generateTimedZapBrief, toggleAutoBriefing } from "../system/ai/automated-zap-brief.js";
...
async function recognizeIntent(message, userContext) {
  try {
    ...
    if (lowerMsg === "aktifkan zap brief" || lowerMsg === "aktifkan brief" ||
        lowerMsg === "turn on zap brief" || lowerMsg === "enable brief") {
      return {
        intent: "toggleBrief",
        enableBrief: true,
        needsContext: false,
    ...
    if (lowerMsg === "nonaktifkan zap brief" || lowerMsg === "matikan brief" ||
        lowerMsg === "turn off zap brief" || lowerMsg === "disable brief") {
      return {
        intent: "toggleBrief",
        enableBrief: false,
        needsContext: false,
...
"requiresDeepAnalysis" = true if requires deep thinking (complex problems, scientific analysis)
    ...
    try {
      ...
      if (result.intent === "toggleBrief") {
        ...
        result.enableBrief = !(message.toLowerCase().includes("nonaktif") || 
...
async function processMessage(m, sock, db) {
  ...
  try {
    ...
    if (intentResult.intent === "toggleBrief") {
      ...
      // Process image
      response = await processImageMessage(messageText, base64Image, userContext);
    } else if (intentResult.intent === "deepThinking" || intentResult.requiresDeepAnalysis) {
      // Use deep thinking model for complex questions
      response = await processDeepThinkingMessage(messageText, intentResult, userContext);
      ...
      // Process reminder
      response = await processReminder(messageText, intentResult, userContext, userId);
    } else if (intentResult.intent === "weather") {
      // Check weather
      response = await getWeatherInfo(intentResult.entities?.locations?.[0] || "Jakarta");
    } else {
      // General conversation
      response = await processChatMessage(messageText, intentResult, userContext);
    }
...
Auto ZAP Brief: ${userContext.preferences?.disableAutomatedBrief ? "Disabled" : "Enabled"}

# IMPORTANT REMEMBERED INFORMATION:
${userContext.memories.slice(-5).map(memory => `- ${memory.content}`).join('\n')}
...
IMPORTANT: If there's no academic question, just return the original question.`
  ...
  try {
    ...
    const thinkingMessages = [
      {
        role: "system",
        content: `You are a highly intelligent and helpful academic assistant, specializing in ${extractedData.subject || "various academic fields"}.
...
Language: Use Indonesian that's easily understood by teenagers/college students, with a casual but informative style.`
    ...
    if (extractedData.relevantContext && extractedData.relevantContext.trim() !== "") {
...
4. The problem requires step-by-step solution`
    ...
    if (analysis.requiresDeepThinking) {
      ...
      const deepThinkingMessages = [
        {
          role: "system",
          content: `You are an academic assistant expert in ${analysis.academicSubject || "various fields"}.
...
If there's text in the image, quote it precisely. If there's a math problem, provide detailed solution with applied formulas. Use casual yet clear language like Indonesian Gen Z style, but with proper reasoning and academic accuracy.`
        },
        {
          ...
          content: [
            {
              type: "text",
              text: message || `Help explain and solve this ${analysis.academicSubject || "academic"} problem`
            },
            {
              type: "image_url",
              image_url: {
        ...
        content: `Create a reminder based on user message. Output JSON:
{
  ...
  "description": "detailed reminder description"
    ...
    let reminderData;
    try {
      reminderData = JSON.parse(reminderCompletion.choices[0].message.content);
    } catch (e) {
...
async function getWeatherInfo(location = "Jakarta") {
  try {
    const weatherPrompt = [
      {
        role: "system",
        content: `Provide weather information for "${location}". Use actual weather forecast data for date ${new Date().toLocaleDateString('id-ID')}.
  ...
  setInterval(async () => {
    try {
      ...
      for (const file of files) {
        ...
        if (!userContext.reminders || userContext.reminders.length === 0) continue;
        
        const now = new Date();
        const todayStr = now.toISOString().split('T')[0];
        
        // Filter reminders for today
        const todayReminders = userContext.reminders.filter(r => r.date === todayStr);
        
        for (const reminder of todayReminders) {
          // Create unique ID for this reminder to avoid duplicate notifications
          const reminderId = `${userId}-${reminder.date}-${reminder.task.substring(0, 10)}`;
          ...
          if (reminder.time) {
            ...
            const isWithinWindow = 
              (currentHour === reminderHour && currentMinute < reminderMinute + 30) ||
              (currentHour === reminderHour + 1 && reminderMinute + 30 > 60 && 
          ...
          try {
            ...
            await whatsappSocket.sendMessage(`${userId}`, { text: reminderMessage });
            console.log(`[REMINDER] Sent to user ${userId}: ${reminder.task}`);
          } catch (error) {
            console.error(`[REMINDER] Error sending to ${userId}:`, error);
          }
        }
      }
    } catch (error) {
