export default handler => {
    handler.reg({
        ...
        run: async (m, { sock, cmds }) => {
            const isOwner = m.isOwner; // Cek jika pengguna adalah owner
            ...
            const filteredCommands = cmds.filter(
                cmd => isOwner || !ownerOnlyCommands.includes(cmd.cmd)
            );
            ...
            filteredCommands.forEach(cmd => {
                ...
                menuPerTag[cmd.tags].push(
                    `│๑ *${prefix}${cmd.cmd}* - ${cmd.desc || "No description"}`
                );
            });
            ...
            if (selectedPage) {
                ...
                sock.sendMessage(
                    m.from,
                    {
                        text: `Halaman ${pageRequested} tidak ditemukan. Total halaman: ${menuPages.length}.`
                    },
                    { quoted: m }
                );
            }
