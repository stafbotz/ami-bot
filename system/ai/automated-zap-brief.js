  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.log(`[ZAP BRIEF] Sent ${timeOfDay} brief to user ${userId}`);
      } catch (error) {
...
async function generateTimedZapBrief(userContext, timeOfDay) {
  try {
    ...
    const zapPrompt = [
      {
        role: "system",
        content: `Create a ${isMorning ? 'Morning' : 'Evening'} ZAP Brief for user named ${userContext.name || 'Friend'}.
...
INSTRUCTIONS:
1. Create informative and concise content tailored for ${isMorning ? 'starting the day' : 'winding down the evening'}
...
7. Use actual ${isMorning ? 'morning' : 'evening'} appropriate information`
    ...
    if (userContext.reminders && userContext.reminders.length > 0) {
      ...
      const relevantReminders = userContext.reminders.filter(r => 
        r.date === (isMorning ? todayStr : tomorrowStr)
      );
      
      if (relevantReminders.length > 0) {
        zapPrompt[0].content += `\n\nUser reminders ${isMorning ? 'for today' : 'for tomorrow'}:\n${relevantReminders.map(r => 
          `- ${r.task} (${r.time ? 'at ' + r.time : 'all day'})`
        ).join('\n')}`;
      }
    }
          ...
          if (memory.entities?.dates) {
            ...
            for (const dateStr of memory.entities.dates) {
              try {
                ...
                if (memoryDate.getDate() === today.getDate() && 
    ...
    return `Maaf, aku nggak bisa menghasilkan ZAP Brief ${timeOfDay === 'morning' ? 'pagi' : 'malam'} saat ini. Coba lagi nanti ya!`;
  }
}
